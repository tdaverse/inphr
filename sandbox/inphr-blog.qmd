...
As expected, the test is not able to reject the null hypothesis. By default, the function uses `B = 1000` permutations. If we compare two samples coming from different geometries (such as trefoil and archspiral), we get:
...
As expected, the test does detect the difference even using only samples of size $5$.
